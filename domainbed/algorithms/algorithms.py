...
class HTCL(AbstractMMD):
    """
    the HTCL algorithm, related with Section 5.2 in the paper
    Note that the complete HTCL will need the process of heterogeneous dividing pattern generation, and that process is not contained in this part
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(HTCL, self).__init__(input_shape, num_classes, num_domains, hparams, gaussian=False)
        
    
    def construct_pairs(self, target_class, features, targets, same_domain):
        """
        this type of constructing pos/neg pairs is the first version, but not used currently
        """
        same_class_feature = []
        for i in range(len(features)):
            if i != same_domain:
                same_class_feature.append(features[i][torch.nonzero(targets[i] == target_class)])
        same_class_feature = torch.cat(same_class_feature, 0) # find feature whose class is the same as `target_class`
        return same_class_feature, \
            features[same_domain][torch.nonzero(targets[same_domain] != target_class)]
            # find feature whose class is different from `target class`
            
    def construct_pairs_v2(self, target_class, features, targets, source_domain):
        """
        return:
        - specific features: feature with the domain same as `source_domain` and class same as `target_class`
        - positive pair: feature with the domain different from `source_domain` and class same as `target_class`
        - negative pair: feature with the domain same as `source_domain` and class different from `target_class`
        """
        same_class_feature = []
        for i in range(len(features)):
            if i != source_domain:
                same_class_feature.append(features[i][torch.nonzero(targets[i] == target_class)])
        same_class_feature = torch.cat(same_class_feature, 0) # find feature whose class is the same as `target_class`
        return features[source_domain][torch.nonzero(targets[source_domain] == target_class)], \
                same_class_feature, \
                features[source_domain][torch.nonzero(targets[source_domain] != target_class)]
                # find feature whose class is different from `target class`

    def mmd_in_batches(self, vec, mat):
        """
        compute the mmd between vec and all of mats' entries
        - vec has the shape of (d,), mat has the shape of (n, d), the return shape is (n, )
        """
        mmds = torch.zeros(mat.shape[0]).to(device)
        for i in range(mat.shape[0]):
            mmds[i] = self.mmd(vec, mat[i])
        return mmds
    ...
    def update(self, x, y, **kwargs):
        ...
        CL_loss = torch.zeros(nmb).to(device)
        for i in range(nmb):
            target_classes = torch.unique(targets[i])
            num_pairs_used = 0
            for j in range(target_classes.shape[0]):
                source, pos_pairs, neg_pairs = self.construct_pairs_v2(target_classes[j], features, targets, i)
                source, pos_pairs, neg_pairs = torch.squeeze(source, 1), torch.squeeze(pos_pairs, 1), torch.squeeze(neg_pairs, 1) # eliminate the 1st dim
                min_dim = min(min(source.shape[0], pos_pairs.shape[0]), neg_pairs.shape[0])
                if min_dim <= 1:
                    continue
                if pos_pairs.shape[0] > min_dim:
                    pos_pairs = pos_pairs[torch.randperm(pos_pairs.shape[0])[0:min_dim]]
                if neg_pairs.shape[0] > min_dim:
                    neg_pairs = neg_pairs[torch.randperm(neg_pairs.shape[0])[0:min_dim]]
                if source.shape[0] > min_dim:
                    source = source[torch.randperm(source.shape[0])[0:min_dim]]
                assert pos_pairs.shape == neg_pairs.shape == source.shape, "Shapes of pos_pairs and neg_pairs differ"
                # compute negative pairs' distances, which is composed of all the anti-class, same-domain features
                neg_dists = self.mmd(source, neg_pairs)
                # compute positive pairs' distances, which is composed of all the same-class, anti-domain features
                pos_dists = self.mmd(source, pos_pairs)
                # compute loss for this target class
                num_pairs_used += source.shape[0]
                CL_loss[i] += - torch.log(neg_dists / (neg_dists + pos_dists))
            if num_pairs_used != 0:
                CL_loss[i] /= num_pairs_used
        num_nozero_loss = torch.nonzero(CL_loss != 0).shape[0] # there is the probability that a batch can't construct complete pos&&neg pairs, so judge the loss in advance
        CL_loss = torch.sum(CL_loss)
        if num_nozero_loss != 0:
            CL_loss = CL_loss / num_nozero_loss
        ...
        self.optimizer.zero_grad()
        (objective + (self.hparams["mmd_gamma"] * penalty) + (self.hparams["CL_weight"] * CL_loss)).backward()
        self.optimizer.step()
        ...
        return {"loss": objective.item(), "penalty": penalty, "CL_loss": CL_loss}
