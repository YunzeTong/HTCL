...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer=None, target_env=None):
    logger.info("")

    #######################################################
    # setup dataset & loader
    #######################################################
    args.real_test_envs = test_envs  # record the index of the test domain in original dataset
    algorithm_class = algorithms.get_algorithm_class(args.algorithm)
    dataset, in_splits, out_splits = get_dataset(test_envs, args, hparams, logger, algorithm_class)
    if args.generate_pattern:
        test_envs = [args.test_env]
    test_splits = []
    if hparams.indomain_test > 0.0:   # 此情况下，所有env被分为train(in_splits), val + test (out_splits)
        ...
        out_splits = val_splits   # indomain_test下, out = val; indomain_test为None下， out = val+test, test_splits = []
    ...
    batch_sizes[test_envs] = 0     # set the batch size of the test domain as 0, then it won't be sampled during training
    ...
    eval_loader_names += ["env{}_inTE".format(i) for i in range(len(test_splits))]  # deafult时，这一项没有加，因为test_splits是空列表
    ...
    for step in range(n_steps):
        ...
        # if step % args.tb_freq == 0:
            ...
            # step_vals["lr"] = scheduler.get_last_lr()[0] # not in SWAD
    ...
    if swad:
        ...
        ret[f"{args.algorithm}_SWAD"] = results["test_in"]
        ret[f"{args.algorithm}_SWAD (inD)"] = results[in_key]
