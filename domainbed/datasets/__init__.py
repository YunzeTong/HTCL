...
from heterolize.main import make_heterogenous_dataset
...
def get_dataset(test_envs, args, hparams, logger, algorithm_class=None):
    ...
    if args.generate_pattern == True:
        add_other_args(args)
        h_dataset = make_heterogenous_dataset(args, logger, hparams=hparams)
        h_dataset.add_params(dataset.num_classes, dataset.N_STEPS, dataset.CHECKPOINT_FREQ, dataset.N_WORKERS, dataset.input_shape)
        dataset = h_dataset
        test_envs = [args.test_env] # not sure, change for colored mnist
        args.test_env = len(dataset) - 1 # set the last domain in the dataset as current test domain
...
def add_other_args(args):
    """
    obtain the index of the real test domain in the original dataset
    this step is necessary because performing heterogeneous pattern generation may change the number of domains
    """
    assert len(args.real_test_envs) == 1
    args.test_env = args.real_test_envs[0] # list -> int
    return args